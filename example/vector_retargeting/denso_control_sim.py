import sys
from leap_hand_sim_driver import run_sim_teleop, run_hand_tracking_and_retargeting
...
def main(
    ...
    image_queue = multiprocessing.Queue(maxsize=2)           # Camera -> Hand-Tracker
    command_queue = multiprocessing.Queue(maxsize=5)         # Hand-Tracker -> Isaac Sim (for robot control)
    annotated_image_queue = multiprocessing.Queue(maxsize=2) # Hand-Tracker -> Isaac Sim (for UI display)

    # --- Process 1: Camera Producer ---
    producer_process = multiprocessing.Process(target=produce_frame, args=(image_queue, camera_path))
    producer_process.start()
    print("🚀 Started Camera Process")

    if sim:
        # --- Process 2: Hand-Tracking (Heavy Lifting) ---
        # MODIFIED: Now also sends the annotated image back
        hand_tracking_process = multiprocessing.Process(
            target=run_hand_tracking_and_retargeting,
            args=(image_queue, command_queue, annotated_image_queue, str(config_path), str(robot_dir))
        )
        hand_tracking_process.start()
        print("🚀 Started Hand-Tracking Process")

        # --- Process 3: Isaac Sim (Simulation & Control) ---
        # MODIFIED: Now also receives the annotated image for display
        sim_process = multiprocessing.Process(
            target=run_sim_teleop,
            args=(command_queue, annotated_image_queue, str(config_path), str(robot_dir), camera_path, gui)
        )
        sim_process.start()
        print("🚀 Started Isaac Sim Process")
        sim_process.join()
        print("Isaac Sim process finished. Terminating other processes...")
        ...
        consumer_process = multiprocessing.Process(
            ...
            args=(image_queue, str(robot_dir), str(config_path), leap_hand),
        )
