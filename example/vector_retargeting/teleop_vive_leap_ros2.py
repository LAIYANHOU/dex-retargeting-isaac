...
def produce_realsense_frame(queue: multiprocessing.Queue):
    ...
    import numpy as np
    import cv2
    from queue import Full # Import the Full exception
    ...
    try:
        while True:
            ...
            # --- MODIFICATION START ---
            try:
                # Use put_nowait() for a non-blocking call.
                queue.put_nowait(color_image)
            except Full:
                # If the queue is full, do nothing. Just drop the frame.
                pass
            # --- MODIFICATION END ---
...
class ViveToROS2Publisher(Node):
    ...
    def reset_origin_callback(self, request, response):
        if self.current_tracker_pos is not None and self.current_tracker_ori is not None and self.robot_current_pose is not None:     
            ...
            print("Resetting tracking origin.", 
                  "tracker_initial_pos:", self.tracker_initial_pos,
                  "tracker_initial_ori:", self.tracker_initial_ori,
